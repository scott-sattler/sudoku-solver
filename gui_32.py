...
class CanvasGUI:
    ...
    def __init__(self, master):
        ...
        self.board_container = Canvas(master)  # height=50.000/9/25*self.dynamic_size, width=225.000/9/25*self.dynamic_size
        ...
        self.canvas_board.config(height=225.000/9/25*self.dynamic_size+self.offset/5+2, width=225.000/9/25*self.dynamic_size+self.offset/5+2)
    ...
    def controls(self):  # TODO standardize the format eg  self.matrix_select["menu"].config(bg='white') vs config
        ...
        for i in range(10):
            spacer = Canvas(self.control_panel, width=50, height=10, bg='white', highlightthickness=2, highlightbackground='white')
    ...
    def create_title(self):
        font_size = self.font_size
        self.title_label = Label(self.board_container, text="SUDOKU SOLVER", font=(self.board_font, font_size*2), bg="#fff")
    ...
    def initialize_board(self):
        ...
        for x_shift in range(BOARD_SIZE-1):
            if (x_shift+1) % 3 != 0:
                self.canvas_board.create_line((x_shift + 1)*cell_size+self.offset/8, 0+self.offset/8, (x_shift + 1)*cell_size+self.offset/8, self.board_canvas_height-465)
            else:
                self.canvas_board.create_line((x_shift + 1)*cell_size+self.offset/10, 0, (x_shift + 1)*cell_size+self.offset/10, self.board_canvas_height, width=3)
        for y_shift in range(BOARD_SIZE-1):
            if (y_shift+1) % 3 != 0:
                self.canvas_board.create_line(0+self.offset/8, (y_shift + 1)*cell_size+self.offset/8, self.board_canvas_height-465, (y_shift + 1)*cell_size+self.offset/8)
            else:
                self.canvas_board.create_line(0, (y_shift + 1)*cell_size+self.offset/8, self.board_canvas_height, (y_shift + 1)*cell_size+self.offset/8, width=3)
        ...
        for x in range(BOARD_SIZE):
            for y in range(BOARD_SIZE):
                ...
                cell = ""
                self.text_id_matrix[y][x] = self.canvas_board.create_text(x_position, y_position, font=(self.board_font, font_size), text=cell)
                # TODO check this [x][y]
        ...
        for i in range(BOARD_SIZE):
            for j in range(BOARD_SIZE):
                if self.canvas_board.itemcget(self.text_id_matrix[i][j], 'text') != "":  # todo check the text value
                    self.shaded_id_matrix[i][j] = self.canvas_board.create_rectangle(j*50+self.offset/8, i*50+self.offset/8, (j+1)*50+self.offset/8, (i+1)*50+self.offset/8, width=0, fill="gray77")
    ...
    # todo move?
    def forget_options_menu(self):  # TODO REMOVE THIS? just use self.select_menu_container.place_forget() where ever it was called
    ...
    def make_mini_boards2(self, matrix, large_square_x, large_square_y): # TODO change name [make] and move
        ...
        for x in range(BOARD_SIZE):
            for y in range(BOARD_SIZE):
                ...
                if cell_zero_check == 0:
                    cell_zero_check = " "
                self.text_id_small_matrix_00[y][x] = self.options_menu_02.create_text(x_position, y_position, font=(self.board_font, font_size), text=cell_zero_check)
    ...
    def make_mini_boards(self, matrix, large_square_x, large_square_y): # TODO change name [make] and move
        ...
        for x in range(BOARD_SIZE):
            for y in range(BOARD_SIZE):
                ...
                if cell_zero_check == 0:
                    cell_zero_check = " "
                self.text_id_small_matrix_00[y][x] = self.canvas_board.create_text(x_position, y_position, font=(self.board_font, font_size), text=cell_zero_check)
                #self.text_id_small_matrix_00[y][x] = Label(font=(self.board_font, font_size), text=cell_zero_check)
                #self.text_id_small_matrix_00[y][x].config(bg='white')
                #self.text_id_small_matrix_00[y][x].place(x=x_position, y=y_position, height=14, width=12)
    ...
    def forget_mini_matrices_menu(self):  # TODO what if I put these all on a canvas then forgot that canvas (would have to redraw lines tho)
        for x in range(9):
            for y in range(9):
